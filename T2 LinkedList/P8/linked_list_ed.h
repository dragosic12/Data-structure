...
public:
    ...
    void agrupar() {
        Nodo* pos1;
        Nodo* act;
        if (prim != nullptr && prim->elem == 'r')
            pos1 = prim;
        else {
           act = prim;
            while (act->sig != nullptr && act->sig->elem != 'r')
                act = act->sig;
            if (act->sig != nullptr) pos1 = act;
            else pos1 = nullptr;
        }
        ...
        while (act != nullptr) {
            ...
            if (act->sig != nullptr && act->sig->elem == 'r') {
    ...
    void colocar(Nodo * pos2, Nodo * pos1) {
        Nodo* a_mover = pos2->sig;
        pos2->sig = a_mover->sig; //a_mover->sig == pos2->sig->sig

        a_mover->sig = pos1->sig;
        pos1->sig = a_mover;
    }
