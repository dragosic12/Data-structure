...
public:
   ...
   std::pair<bool, T> lower_bound(T const& e) const {
       T aux = -1;
       buscaLower(raiz, e, aux);
       if (aux < e) {
           return std::pair<bool, T>(false, aux);
       }
       else {
           return std::pair<bool, T>(true, aux);
       }
...
protected:
   ...
   static void buscaLower(Link p, const T& e, T& aux) {
       ...
       if (p != NULL) {
           ...
           buscaLower(p->iz, e, aux);
           buscaLower(p->dr, e, aux);
